...
const selectAnswer = (option) => {
  if (currentQuestion.value) {
    ...
    // Handle Crecy selection for both QE1 and QS1
    if (questionId === "QE1" && option.id === 1) {
      answers.value[`QE1`] = 1; // Explicitly save QE1 as 1
      answers.value[`QE1_COMMUNE`] = "CRECY LA CHAPELLE";
      answers.value[`QE1_CODE_INSEE`] = "77142";
      answers.value[`QE1_COMMUNE_LIBRE`] = "";
    } else if (questionId === "QS1" && option.id === 1) {
      answers.value[`QS1`] = 1; // Explicitly save QS1 as 1
      answers.value[`QS1_COMMUNE`] = "CRECY LA CHAPELLE";
      answers.value[`QS1_CODE_INSEE`] = "77142";
      answers.value[`QS1_COMMUNE_LIBRE`] = "";
    } else if (
      (questionId === "QE1" || questionId === "QS1") &&
      option.id === 2
    ) {
      // For "Autres, à préciser", save the option ID but let the commune selector handle the commune details
      answers.value[questionId] = option.id;
    } else {
      // For all other questions, save the answer normally
      answers.value[questionId] = option.id;
    }
...
const finishSurvey = async () => {
  ...
  Object.keys(answers.value).forEach((key) => {
    ...
    if (answers.value[key] !== undefined && answers.value[key] !== null) {
      ...
      console.warn(`Skipping undefined or null value for field: ${key}`);
    }
  });
...
const getNextId = async () => {
  ...
  return `antiers-${counter.toString().padStart(6, "0")}`;
};
</script>
